...
async function get_projectPriceLineItem(filterVariable, project_version_id) {
    try {
        //await sql.connect(config);
        // Load the query from the text file
        const queryFilePath = path.join(__dirname, 'QueryList', 'query_projectPriceLineItem.sql'); // Path to query file in subfolder
...
const result = await executeQuery(query);
        ...
        console.log(query)
        console.error('SQL Error: 22', error);
        throw error;
    } finally {
        //await sql.close();
    }
}
module.exports = get_projectPriceLineItem;