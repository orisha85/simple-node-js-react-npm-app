...
async function ratetypevalue(filterVariable) {
    try {
        //await sql.connect(config);

        // Load the query from the text file
        const queryFilePath = path.join(__dirname, 'QueryList', 'query_ratetypevalue.sql'); // Path to query file in subfolder
...
const result = await executeQuery(query);
        ...
        console.error('SQL Error: 26', error);
        throw error;
    } finally {
        //await sql.close();
    }
}
module.exports = ratetypevalue;