...
async function activity(filterVariable) {
    try {
        ...
        query += ` WHERE IsActive=1 AND [Tenant_CD] = '${filterVariable}' AND [Category_CD] = 'AC' ORDER BY SORTORDER ASC`; // Modify [SomeColumn] with the actual column name
...
const result = await executeQuery(query);
        ...
        console.error('SQL Error:4 ', error);
        throw error;
    } finally {
        //await sql.close();
    }
