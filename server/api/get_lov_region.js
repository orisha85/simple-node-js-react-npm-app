...
async function region(filterVariable) {
    try {
        ...
        query += ` WHERE IsActive=1 AND [Tenant_CD] = '${filterVariable}' AND [Category_CD] = 'RG' ORDER BY SORTORDER ASC`; // Modify [SomeColumn] with the actual column name
...
const result = await executeQuery(query);
        ...
        console.error('SQL Error: 9', error);
        throw error;
    } finally {
        //await sql.close();
    }
