    ...
    data.forEach(item => {
        ...
        if (priceSummary) {
            Object.values(aggregatedData).forEach(item => {
                ...
                //console.log("Pri", priceSummary)
                //console.log("Aggregated Data", aggregatedData)
                const dbRecord = priceSummary ? priceSummary.find(obj => obj.Header_ID == item.Header_ID && obj.SKU_Alias_Item_ID == item.SKU_Item_ID) : undefined;
...
export function updateBidPricingTable(priceSummary, setTotalBidPrice) {
    ...
    const totalPrice = bidPricingTable.getCalcResults().bottom.Total_Price;
    //console.log("Pricingggg", totalPrice)
