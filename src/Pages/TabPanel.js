...
class Tab extends React.Component {
  ...
  render() {
    const tabClassName = this.props.isSelected
      ? 'tab tab--selected'
      : 'tab';

    return (
      <span className={tabClassName} onClick={this._handleClick}>
      <span className="tab__label" style={{ fontFamily: 'inherit', textTransform: 'capitalize', fontSize: '15px' }}>{this.props.label}</span>
      </span>
    );
  }
}
