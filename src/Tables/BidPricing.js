    ...
    useEffect(() => {
        if (el.current && !tabulator.current) {
            const columns = [
                ...
                { title: "<span style='color: #007bff;'>✎</span> Quoted Qty", field: 'Quantity_Actual', hozAlign: "center", editor: "number", editable: !isDisabled, validator:"max:1000000", formatter: function (cell) { 
                    return Helper.formatNumber(cell.getValue(), 0, 2, 0) 
            ...
            {
                title: 'Total Price', field: 'Total_Price', hozAlign: "center", bottomCalc: "sum", mutator: function (value, data) {
                    const qty = data.Quoted_Qty > 0 ? data.Quoted_Qty : data.Mat_Quantity_Actual
