...
async function activity(filterVariable, UoM) {
    try {
        ...
        query += ` WHERE IsActive=1 AND [Tenant_CD] = '${filterVariable}' AND [Item_CD] = '${UoM}' ORDER BY SORTORDER ASC`; // Modify [SomeColumn] with the actual column name

        // Execute the query
        const result = await sql.query(query);
        return result.recordset;
    } catch (error) {
        console.error('SQL Error: 7', error);
        throw error;
    } finally {
        await sql.close();
    }
