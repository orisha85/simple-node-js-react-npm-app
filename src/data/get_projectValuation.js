...
async function get_projectValuation(filterVariable) {
    try {
        await sql.connect(config);
        // Load the query from the text file
        const queryFilePath = path.join(__dirname, 'QueryList', 'query_projectValuation.sql'); // Path to query file in subfolder
        ...
        console.error('SQL Error: 24', error);
        throw error;
    } finally {
        await sql.close();
    }
}
module.exports = get_projectValuation;