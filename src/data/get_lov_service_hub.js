...
async function serviceHub(filterVariable) {
    try {
        ...
        query += ` WHERE IsActive=1 AND [Tenant_CD] = '${filterVariable}' AND [Category_CD] = 'SH' ORDER BY SORTORDER ASC`; // Modify [SomeColumn] with the actual column name

        // Execute the query
        const result = await sql.query(query);
        return result.recordset;
    } catch (error) {
        console.error('SQL Error: 10', error);
        throw error;
    } finally {
        await sql.close();
    }
