...
async function get_projectCostHeader(filterVariable, project_version_id) {
    try {
        await sql.connect(config);
        // Load the query from the text file
        const queryFilePath = path.join(__dirname, 'QueryList', 'query_projectCostHeader.sql'); // Path to query file in subfolder
        ...
        console.error('SQL Error: 18', error);
        throw error;
    } finally {
        await sql.close();
    }
}
module.exports = get_projectCostHeader;